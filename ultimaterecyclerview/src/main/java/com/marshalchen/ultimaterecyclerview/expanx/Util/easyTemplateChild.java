...
public abstract class easyTemplateChild<T extends ExpandableItemData, B extends TextView, H extends RelativeLayout> extends child<T> {
    ...
    public easyTemplateChild(View itemView) {
        ...
        im=(ImageView)itemView.findViewById(R.id.exp_section_im);
    ...
    @Override
    public void bindView(final T itemData,final int position) {
...
//    }
    ...
    @Override
    public void onChildItemClick(ExpandableItemData data) {
        String[] v = data.getPath().split("/");
        if (v.length > 1) {
            request_api(v, data.getText());
        }
    }
